...
class userController():
    ...
    def get_file_user(self, request, fileId):

        # verofoicamos la existencia del archivo
        ...
        static_files =  app_dir+StaticFiles(directory=os.getenv("FILES_USERS"))
        ...
        if (nRecordFileUser>0):
            ...
            resultado={
                ...
                "absolute_path":"file://"+static_files+result.url
            }
            if (result):
...
#
    # metodo para consultar un archivo por Id
    # @params fileId: id del archivo del Usuario que se desea consultar
    def list_files_users(self, request, userId):

        # Obtener la dirección del servidor.
        result= self.db.query(ArchivosUsuariosModel).filter(ArchivosUsuariosModel.user_id==userId).all()

        main_file = os.path.abspath(__file__)
        app_dir = os.path.dirname(main_file)+"/.."

        # Crear la instancia de StaticFiles.
        static_files = StaticFiles(directory=os.getenv("FILES_USERS"))
