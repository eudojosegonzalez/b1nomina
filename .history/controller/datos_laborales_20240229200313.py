...
class DatosLaboralesController():
    ...
    def create_datos_laborales(self, datosLaborales:DatosLaboralesSchema, userCreatorId:int ):
        ...
        nRecordDatosLaborales=  self.db.query(DatosLaboralesModel).filter(DatosLaboralesModel.user_id == datosLaborales.user_id).count()

        if (nRecordDatosLaborales > 0):
            datoLaboralExists = self.db.query(DatosLaboralesModel).filter(DatosLaboralesModel.user_id == datosLaborales.user_id).first()
            return ({"result":"-1","estado":"Este usuario ya tiene datos laborales no puede volver a crearlo","DatoLaboral": datoLaboralExists})     
