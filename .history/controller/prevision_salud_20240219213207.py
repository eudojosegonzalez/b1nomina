...
class PrevisionSaludController():
    ...
    def create_historico_prevision_salud(self, prevision:PrevisionSaludSchema, observacion:str):
        ...
        '''
        id = Column(BIGINT, primary_key=True, autoincrement=True, nullable=False)
        prevision_id = Column(BIGINT,  nullable=False)    
        codigo_externo = Column(VARCHAR(50), unique=True, nullable=False)
        nombre = Column(VARCHAR(100), nullable=False)
        prevision_salud_cuenta = Column(VARCHAR(150), nullable=False)
        codigo_direccion_trabajo = Column(VARCHAR(10), nullable=True)
    ...
    def create_prevision_salud(self, sede:SedeSchema, userCreatorId:int ):
        #obtenemos la fecha/hora del servidor
        ahora=datetime.datetime.now()

        nombreSede=prevision.nombre.upper().strip()
        ...
        if (nRecordNombre > 0):
            ...
            try:
                newSede=SedeModel(
                    sociedad_id=prevision.sociedad_id,
                    nombre=((prevision.nombre).upper()).strip(),
                    region_id=prevision.region_id,
                    comuna_id=prevision.comuna_id,
                    direccion=((prevision.direccion).upper()).strip(),
                    ciudad=((prevision.ciudad).upper()).strip(),
                    created=ahora,
                    updated=ahora,
