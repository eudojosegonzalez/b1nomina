...
class sedesController():
    ...
    def create_sede(self, sede:SedeSchema, userCreatorId:int ):
        ...
        if (nRecordNombre > 0):
            ...
            sedeExists=self.db.query(SedeModel).filter(SedeModel.nombre == nombreSociedad).first() 

            # devolvemos la sociedad que ya existe
            return ({"result":"-1","estado":"Existe una sociedad con ese nombre","data":sedeExists})
        elif (nRecordRut > 0):
            # buscamos la sociedad con el nombre y lo devolvemos
            sedeExists=self.db.query(SedeModel).filter(SedeModel.rut == rutSociedad).first() 

            # devolvemos la sociedad que ya existe
            return ({"result":"-1","estado":"Existe una sociedad con ese rut","data":sedeExists})            
        else:    
    ...
    def update_sede(self, sede:SedeSchema, userUpdaterId:int, id:int):
        ...
        if (nRecord == 0):
            ...
            try:
                ...
                self.create_historico_sede(sedeExists ,"Actualización de la data de la sociedad")

                #registramnos los cambios en la tabla de bancarios del usuario
                sedeExists.sociedad_id=((sede.sociedad_id).upper()).strip(),
                ...
                return ({"result":"1","estado":"Se actualizó el dato de de la sede","data":data})
            except ValueError as e:
                return( {"result":"-3","error": str(e)})                    
