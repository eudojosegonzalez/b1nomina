...
class contactUserController():
    ...
    def create_historico_bancario_user (self, bancarioUser: BancariosUserModel, observacion:str):
     # determinamos la fecha/hora actual
        ...
        try:
            ...
            newHistoricoBancarioUser= historicoContactUserModel(
                ...
                rut_tercero = bancarioUser.terceros ,
    ...
    def create_bancario_user(self, userCreatorId:int ,bancarioUsuario:BancariosUserSchema):
        #obtenemos la fecha/hora del servidor
        ahora=datetime.datetime.now()
        
        # determinamos el user_id de los datos enviados
        bancarioUserId=bancarioUsuario.user_id

        #creamos el nuevo registro de bancarios de usuarios
        try:
            newBancarioUser=BancariosUserModel(
                user_id=bancarioUsuario.user_id,
                bancario_id=bancarioUsuario.id,
                user_id=bancarioUsuario.user_id,     
                banco_id=bancarioUsuario.banco_id ,
                numero_cuenta=bancarioUsuario.numero_cuenta,
                en_uso =bancarioUsuario.en_uso,
                terceros = bancarioUsuario.terceros ,
                rut_tercero = bancarioUsuario.terceros ,
            ...
            self.create_historico_contact_user(newBancarioUser,"Se creó la data de contacto del usuario")
    ...
    # @userUpdaterId: Id del usuario que está actualizando el registro
    # @params contactoUsuario: esquema de los datos de contacto del usuario que se desea insertar       
    def update_bancario_user(self,userUpdaterId:int ,contactoUsuario:contactUserSchema):
        #obtenemos la fecha/hora del servidor
        ahora=datetime.datetime.now()
