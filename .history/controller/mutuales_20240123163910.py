...
class mutualesController():
    ...
    def create_mutual(self, mutual:mutualesSchema, userCreatorId:int ):
        #obtenemos la fecha/hora del servidor
        ahora=datetime.datetime.now()

        nombreBanco=mutual.nombre.upper().strip()
        codigoBanco=mutual.codigo.upper().strip()


        # contamos si existe un banco con el mismo nombre
        nRecordNombre = self.db.query(mutualesModel).filter(mutualesModel.nombre == nombreBanco).count()  

        # contamos si existe un banco con el mismo código 
        nRecordCodigo = self.db.query(mutualesModel).filter(mutualesModel.codigo == codigoBanco).count()  

        if (nRecordNombre > 0):
            # buscamos el banco con el nombre y lo devolvemos
            bancoExists=self.db.query(mutualesModel).filter(mutualesModel.nombre == nombreBanco).first() 
            ...
            bancoExists=self.db.query(mutualesModel).filter(mutualesModel.codigo == codigoBanco).first() 
            ...
            try:
                newBanco=mutualesModel(
                    codigo=((mutual.codigo).upper()).strip(),
                ...
                self.create_historico_mutuales(newBanco,"Se creó un banco en el sistema")
  
                data={
                    "id":newmutual.id,
                    "codigo": newmutual.codigo,
                    "nombre": newmutual.nombre,
                    "nomina": newmutual.nomina,
    ...
    def search_mutual(self,finding ,page, records):
    ...
    def update_mutual(self, mutual:mutualesSchema, userUpdaterId:int, id:int):
        #obtenemos la fecha/hora del servidor
        ahora=datetime.datetime.now()
    ...
    def list_history_mutuales(self, page:int, records: int, id:int):
        ...
        nRecord = self.db.query(HistoricomutualesModel).filter(HistoricomutualesModel.banco_id == id).count()
        ...
        if (nRecord == 0):
            ...
            try:
                # ejecutamos la consulta
                consulta = self.db.query(HistoricomutualesModel).filter(HistoricomutualesModel.banco_id == id)
                consulta = consulta.limit(records)
                consulta = consulta.offset(records * (page - 1))
                listHistorymutuales=consulta.all()
               
                # se actualizó el registro devolvemoslos registros encontrados
                return ({"result":"1","estado":"Se consiguieron los datos historicos del banco ","data": listHistorymutuales})
            except ValueError as e:
                # ocurrió un error devolvemos el error
                return( {"result":"-1","error": str(e)})     