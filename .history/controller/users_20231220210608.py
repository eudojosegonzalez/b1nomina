    ...
    def update_user (self, userId: int, data : UserSchema ):
        try:       
            ...
            if (user):
                ...
                user.updater_user=userId
                self.db.commit()
                # se actualizó la data personal del usuario
                return ({"result":"1","estado":"Usuario Actualizado","UserId":id,"data":data})
            else:
                # no existe el ID del usuario
                return ({"result":"-1","estado":"Usuario no encontrado","UserId":id,"data":data})
        except ValueError as e:
                return( {"result":"-3","error": str(e),"data":data})
