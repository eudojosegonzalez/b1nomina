...
class DatosLaboralesController():
    ...
    def create_datos_laborales(self, datosLaborales:DatosLaboralesSchema, userCreatorId:int ):
        ...
        try:
            ...
            self.create_historico_datos_laborales(newDatosLaborales,"Se creó una sede en el sistema")
    ...
    def get_datos_laborales(self,id:int):
        ...
        if (nRecord == 0):
            ...
            try:
                ...
                return ({"result":"1","estado":"Se consiguieron los dtos laborales","data":sedeExits})
            except ValueError as e:
    ...
    #metodo para consultar los datos Laborales por sociedad_id
    def get_datos_laborales_sociedad(self,Id:int):

        # buscamos si este existe esta sede
        nRecord = self.db.query(DatosLaboralesModel).filter(DatosLaboralesModel.sociedad_id == Id).count()
        
        if (nRecord == 0):
            # no existen datos laborales de este usuario
            return ({"result":"-2","estado":"No record found"})
        else:
            # se extraen los datos laborales de esta sociedad
            try:
                DatosLaboralesExits = self.db.query(DatosLaboralesModel).filter(DatosLaboralesModel.sociedad_id == Id).all()                  
                # devolvemos los datos laborales
                return ({"result":"1","estado":"Se consiguieron los dtos laborales","data":DatosLaboralesExits})
            except ValueError as e:
                # ocurrió un error devolvemos el error
    ...
    def get_datos_laborales_sede(self,Id:int):
    ...
    def get_datos_laborales_departamento(self,Id:int):
    ...
    def get_datos_laborales_grupo(self,Id:int):
    ...
    def list_datos_laborales(self, page, records):
    ...
    # @params page: pagina de los datos que se mostrará
    # @params records: cantidad de registros por página
    def list_datos_laborales_sociedad(self,idSociedad, ):
        consulta = self.db.query(DatosLaboralesModel).filter(DatosLaboralesModel.sociedad_id==idSociedad)
        result=consulta.all()
        return (result)
