...
class PeriodosController():
    ...
    def create_historico_periodos(self, periodos: PeriodosModel, observacion:str):
    ...
    def create_periodos(self, Periodos:PeriodosSchema, userCreatorId:int ):
        ...
        if (nRecordNombre > 0):
            ...
            return ({"result":"-1","estado":"Existe una sociedad con ese nombre","data":PeriodosExists})          
        else:    
            #creamos el nuevo registro de banco
            try:
                newPeriodos=PeriodosModel(
                    sociedad_id=Periodos.sociedad_id,
                    nombre=((Periodos.nombre).upper()).strip(),
                    region_id=Periodos.region_id,
                    comuna_id=Periodos.comuna_id,
                    direccion=((Periodos.direccion).upper()).strip(),
                    ciudad=((Periodos.ciudad).upper()).strip(),
                    created=ahora,
                    updated=ahora,
                ...
                self.create_historico_Periodos(newPeriodos,"Se creó una sociedad en el sistema")
  
                data={
                    "sociedad_id":newPeriodos.sociedad_id,
                    "nombre": newPeriodos.nombre,
                    "region_id":newPeriodos.region_id,
                    "comuna_id":newPeriodos.comuna_id,
                    "ciudad":newPeriodos.ciudad,
                    "direccion":newPeriodos.direccion,
    ...
    def get_periodos(self,id:int):
        ...
        if (nRecord == 0):
            ...
            try:
                ...
                return ({"result":"1","estado":"Se consiguieron los datos de la Periodos","data":PeriodosExits})
            except ValueError as e:
                # ocurrió un error devolvemos el error
                return( {"result":"-1","error": str(e)}) 
            

    #metodo para efectuar búsquedas en las Periodoss
    # @params cadena: cadena que se buscara en la tabla Periodoss comparando con el campo nombre 
    def search_periodos(self,finding ,page, records):

        findingT="%"+finding+"%"

        try:
            # buscamos si hay resgistros coincidentes
            nRecord=self.db.query(PeriodosModel).filter(PeriodosModel.nombre.like(findingT) | PeriodosModel.direccion.like(findingT)).count()  

            # hay registros
            if (nRecord >0):
                # ejecutamos la consulta
                consulta = self.db.query(PeriodosModel).filter(PeriodosModel.nombre.like(findingT) | PeriodosModel.direccion.like(findingT))
    ...
    def list_periodos(self, page, records):
        consulta = self.db.query(PeriodosModel)
        consulta = consulta.limit(records)
        consulta = consulta.offset(records * (page - 1))
        result=consulta.all()
        return (result)
