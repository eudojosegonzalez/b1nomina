    ...
    def update_user (self, user_updater: int, data : UserSchema ):
        try:       
            ...
            if (user):
                ...
                nRecordUsername=self.db.query(UsuarioModel).filter((UsuarioModel.username == data.username) and (UsuarioModel.id != data.user)).count()
                nRecordUsername=self.db.query(UsuarioModel).filter_by((UsuarioModel.username == data.username) ,(UsuarioModel.id != data.user)).count()

                #veiricamos que ese rut no este registrado en el sistema
                nRecordRut=self.db.query(UsuarioModel).filter((UsuarioModel.rut == data.rut) and (UsuarioModel.id != data.user)).count() 

                #veiricamos que ese rut_provisorio no este registrado en el sistema
                nRecordRutProvisorio=self.db.query(UsuarioModel).filter((UsuarioModel.rut_provisorio==data.rut_provisorio) and (UsuarioModel.id != data.user)).count()                                                       
