...
class contactUserController():
    ...
    #metodo para insertar  los datos personales del usuario   
    # @params usuario: esquema de los datos del usuario que se desea insertar       
    def update_contact_user(self,userCreatorId:int ,contactoUsuario:contactUserSchema):
        #obtenemos la fecha/hora del servidor
        ahora=datetime.datetime.now()
        ...
        if not (userContactExists):
            # el contacto del usuario ya existe no puede volver a crearlo
            return ({"result":"-2","estado":"Record not found","userId": userContactExists.id })
        ...
        try:
            ...
            newcontactUserId=ContactUser.id
            return ({"result":"1","estado":"actualizado","contactUserId":contactUserId})
        except ValueError as e:
            return( {"result":"-1","error": str(e)})        
