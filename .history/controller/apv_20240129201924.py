...
class APVController():
    ...
    def search_apv(self,finding ,page, records):
        ...
        try:
            ...
            nRecordNombre=self.db.query(APVModel).filter(APVModel.nombre.like(findingT) ).count()  

            # buscamos si hay resgistros coincidentes por nombre
            nRecordNombreL=self.db.query(APVModel).filter(APVModel.nombre_largo.like(findingT) ).count()     

            # buscamos si hay resgistros coincidentes por codigo_externo
            nRecordCodigo=self.db.query(APVModel).filter(APVModel.codigo_externo.like(findingT) ).count()                          

            # hay registros
            if (nRecordNombre >0):
                # ejecutamos la consulta
                consulta = self.db.query(APVModel).filter(APVModel.nombre.like(findingT)).first()
            ...
            elif (nRecordNombreL > 0):
                # ejecutamos la consulta
                consulta = self.db.query(APVModel).filter(APVModel.nombre_largo.like(findingT)).first()
                consulta = consulta.limit(records)
                consulta = consulta.offset(records * (page - 1))
                result=consulta.all()
                ...
                # ejecutamos la consulta
                consulta = self.db.query(APVModel).filter(APVModel.codigo_externo.like(findingT)).first()
                consulta = consulta.limit(records)
                consulta = consulta.offset(records * (page - 1))
                result=consulta.all()
