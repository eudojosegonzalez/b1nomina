...
class bancosController():
    ...
    def create_historico_unidades_pacto (self, unidadPacto: UnidadesPactoModel, observacion:str):
        ...
        try:
            ...
            newHistoricoUnidaesPacto= HistoricoUnidadesPactoModel(
                ...
                banco_id = unidadPacto.id,
                codigo = unidadPacto.codigo,
                nombre = unidadPacto.nombre,
                nomina = unidadPacto.nomina,
    ...
    def create_unidadPacto(self, banco:BancosSchema, userCreatorId:int ):
        #obtenemos la fecha/hora del servidor
        ahora=datetime.datetime.now()

        nombreBanco=unidadPacto.nombre.upper().strip()
        codigoBanco=unidadPacto.codigo.upper().strip()


        # contamos si existe un banco con el mismo nombre
        nRecordNombre = self.db.query(UnidadesPactoModel).filter(UnidadesPactoModel.nombre == nombreBanco).count()  

        # contamos si existe un banco con el mismo código 
        nRecordCodigo = self.db.query(UnidadesPactoModel).filter(UnidadesPactoModel.codigo == codigoBanco).count()  

        if (nRecordNombre > 0):
            # buscamos el banco con el nombre y lo devolvemos
            bancoExists=self.db.query(UnidadesPactoModel).filter(UnidadesPactoModel.nombre == nombreBanco).first() 
            ...
            bancoExists=self.db.query(UnidadesPactoModel).filter(UnidadesPactoModel.codigo == codigoBanco).first() 
            ...
            try:
                newBanco=UnidadesPactoModel(
                    codigo=((unidadPacto.codigo).upper()).strip(),
                    nombre=((unidadPacto.nombre).upper()).strip(),
                    nomina=unidadPacto.nomina,
                    created=ahora,
                    updated=ahora,
                ...
                self.create_historico_unidades_pacto(newBanco,"Se creó un banco en el sistema")
  
                data={
                    "id":newunidadPacto.id,
                    "codigo": newunidadPacto.codigo,
                    "nombre": newunidadPacto.nombre,
                    "nomina": newunidadPacto.nomina,
    ...
    def get_unidadPacto(self,id:int):
    ...
    def search_unidadPacto(self,finding ,page, records):
    ...
    def update_unidadPacto(self, unidadPacto:UnidadesPactoSchema, userUpdaterId:int, id:int):
        #obtenemos la fecha/hora del servidor
        ahora=datetime.datetime.now()
        ...
        if (nRecord == 0):
            ...
            try:
                ...
                unidadPactoExists.codigo=((unidadPacto.codigo).upper()).strip(),
                unidadPactoExists.nombre=((unidadPacto.nombre).upper()).strip(),
                unidadPactoExists.updated=ahora,
                unidadPactoExists.updater_user=userUpdaterId               

                if (unidadPacto.nomina==1):
                    unidadPactoExists.nomina=True
                else:
                    unidadPactoExists.nomina=False
    ...
    def list_history_unidades_pacto(self, page:int, records: int, id:int):
