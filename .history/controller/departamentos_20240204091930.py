...
class sedesController():
    ...
    def create_historico_departamento(self, departamento: DepartamentosSchema, observacion:str):
        ...
        try:
            ...
            newHistoricoSociedad= HistoricoDepartamentosModel(
                sede_id=departamento.id,
    ...
    def create_departamento(self, departamento:DepartamentosSchema, userCreatorId:int ):
        ...
        if (nRecordNombre > 0):
            ...
            try:
                ...
                self.create_historico_departamento(newSede,"Se creó una sociedad en el sistema")
    ...
    def search_departamentos(self,finding ,page, records):
        ...
        try:
            ...
            nRecord=self.db.query(DepartamentosModel).filter(DepartamentosModel.nombre.like(findingT) | DepartamentosModel.direccion.like(findingT)).count()  

            # hay registros
            if (nRecord >0):
                # ejecutamos la consulta
                consulta = self.db.query(DepartamentosModel).filter(DepartamentosModel.nombre.like(findingT) | DepartamentosModel.direccion.like(findingT))
    ...
    def list_departamentos_sociedad(self, page, records):
        consulta = self.db.query(DepartamentosModel)
        consulta = consulta.limit(records)
        consulta = consulta.offset(records * (page - 1))
        result=consulta.all()
        return (result)

    # metodo para consultar todas los departamentos de una sociedad
    # @params page: pagina de los datos que se mostrará
    # @params records: cantidad de registros por página
    ...
    def list_history_departamentos(self, page:int, records: int, id:int):
        ...
        nRecord = self.db.query(HistoricosDepartamentosModel).filter(HistoricosDepartamentosModel.id == id).count()
        ...
        if (nRecord == 0):
            ...
            try:
                # ejecutamos la consulta
                consulta = self.db.query(HistoricosDepartamentosModel).filter(HistoricosDepartamentosModel.sociedad_id == id)
                consulta = consulta.limit(records)
                consulta = consulta.offset(records * (page - 1))
