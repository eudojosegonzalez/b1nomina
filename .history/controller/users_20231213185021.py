...
class userController():
    ...
    def create_user(self, usuario:UserSchema):
        ...
        # buscamos si el rut o el rut provisiorio ya existe
        rutV=usuario.rut
        rutProvisorio=usuario.rut_provisorio
        ...
        if (userExistsUserName):
            # el username esta repetido
            return ({"result":"-2","estado":"Username ya existe, no puede volver a crearlo","userId": userExists.id,"userName":userNameDB })

        if ((userExistsUserRut) or (userExistsUserRutProvisorio))
            # el rut ya existe
            return ({"result":"-3","estado":"Este Rut ya existe en la Base de Datos, no puede volver a crearlo","userId": userExists.id,"rut":rutV })                
    ...
    def update_user (self, userId: int, data : UserSchema ):
        try:        
            ...
            updater_user=userId
            self.db.commit()
            return ({"result":"1","estado":"Usuario Actualizado","UserId":userId})
        except ValueError as e:
                return( {"result":"-1","error": str(e)})
