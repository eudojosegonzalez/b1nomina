...
class userController():
   ...
   #metodo para insertar  los datos personales de preregistro del usuario   
    # @params preUser: esquema de los datos del usuario que se desea insertar       
    def update_pre_user2(self, idUser, preUser:PreUserSchema):
        #obtenemos la fecha/hora del servidor
        ahora=datetime.datetime.now()
        ...
        nRecordUserRut = self.db.query(UsuarioModel).filter(UsuarioModel.rut == rutV and UsuarioModel.id != idUser).count()


        # contamos si existe un rut identico en la base de datos 
        nRecordUserRutProvisiorio = self.db.query(UsuarioModel).filter(UsuarioModel.rut_provisorio == rutV and UsuarioModel.id != idUser).count()        

        #verificamos si existe o no el rut y el rutprovisorio
        if ((nRecordUserRut > 0 ) or (nRecordUserRutProvisiorio > 0)):
            # el rut ya existe
            userExistsUserRut = self.db.query(UsuarioModel).filter(UsuarioModel.rut == rutV).first()
            return ({"result":"-3","estado":"Este Documento ya existe en la Base de Datos, no puede volver a asignarlo a otro empleado","user": userExistsUserRut})                
        ...
        if (rutOk and  nombresOk and apellidosOk  and correoOk):
            ...
            try:
                # creamos el registro de contacto
                userContactExist=self.db.query(contactUserModel).filter(contactUserModel.id==idUser).first()
                userContactExist.email=preUser.correo,
                userContactExist.updated=ahora,
                userContactExist.updater_user=1
                self.db.commit()
