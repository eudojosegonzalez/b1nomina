...
class DatosPagoController():
    ...
    def create_historico_datos_pago(self, datosPago: DatosPagoModel, observacion:str):
        ...
        try:
            ...
            newHistoricoSociedad= HistoricoDatosPagoModel(
                datos_laborales_id=datosPago.id,
                sociedad_id=datosPago.sociedad_id,
                sede_id=datosPago.sede_id,
                departamento_id=datosPago.departamento_id,
                grupo_id=datosPago.grupo_id,
                cargo_id=datosPago.cargo_id,
                user_id=datosPago.user_id,
                tipo_contrato=datosPago.tipo_contrato,
                termino_contrato=datosPago.termino_contrato,
                fecha_inicio=datosPago.fecha_inicio,
                fecha_fin=datosPago.fecha_fin,
                periodo_salario=datosPago.periodo_salario,
                modalidad=datosPago.modalidad,
                dias_descanso=datosPago.dias_descanso,
                salario_base=datosPago.salario_base,
    ...
    def create_datos_pago(self, datosPago:DatosPagoSchema, userCreatorId:int ):
        ...
        if (nRecordDatosLaborales > 0):
            ...
            try:
                ...
                self.create_historico_datos_pago(newDatosPago,"Se creó un Dato Laboral en el sistema")
    ...
    # metodo para consultar todas los Datos de Pago
    def list_datos_pago_sociedad(self,Id ):
        consulta = self.db.query(DatosPagoModel).filter(DatosPagoModel.sociedad_id==Id)
        result=consulta.all()
        return (result)
    ...
    def list_datos_pago_sede(self,Id:int):

        # buscamos si este existe esta sede
        nRecord = self.db.query(DatosPagoModel).filter(DatosPagoModel.sede_id == Id).count()
        ...
        if (nRecord == 0):
            ...
            # se extraen los datos laborales de este usuario
            try:
                DatosLaboralesExits = self.db.query(DatosPagoModel).filter(DatosPagoModel.sede_id == Id).all()                  
                # devolvemos los datos laborales
    ...
    def list_datos_pago_departamento(self,Id:int):

        # buscamos si este existe esta sede
        nRecord = self.db.query(DatosPagoModel).filter(DatosPagoModel.departamento_id == Id).count()
        ...
        if (nRecord == 0):
            ...
            try:
                DatosLaboralesExits = self.db.query(DatosPagoModel).filter(DatosPagoModel.departamento_id == Id).all()                  
                # devolvemos los datos laborales
                return ({"result":"1","estado":"Se consiguieron los dtos laborales","data":DatosLaboralesExits})
            except ValueError as e:
                # ocurrió un error devolvemos el error
    ...
    def list_datos_pago_grupo(self,Id:int):

        # buscamos si este existe esta sede
        nRecord = self.db.query(DatosPagoModel).filter(DatosPagoModel.grupo_id == Id).count()
        ...
        if (nRecord == 0):
            ...
            try:
                DatosLaboralesExits = self.db.query(DatosPagoModel).filter(DatosPagoModel.grupo_id == Id).all()                  
                # devolvemos los datos laborales
                return ({"result":"1","estado":"Se consiguieron los dtos laborales","data":DatosLaboralesExits})
            except ValueError as e:
                # ocurrió un error devolvemos el error
    ...
    def update_datos_pago(self, datosPago:DatosPagoSchema, userUpdaterId:int, id:int):
        ...
        if (nRecord == 0):
            ...
            try:
                ...
                datosLaboralesExists = self.db.query(DatosPagoModel).filter(DatosPagoModel.id == id).first()             

                #creamos el registro historico de datos laborales
                self.create_historico_datos_pago(datosLaboralesExists ,"Actualización de la data de la sede")

                #registramnos los cambios en la tabla de datos laborales
                datosLaboralesExists.sociedad_id=datosPago.sociedad_id,
                datosLaboralesExists.sede_id=datosPago.sede_id,
                datosLaboralesExists.departamento_id=datosPago.departamento_id,
                datosLaboralesExists.grupo_id=datosPago.grupo_id,
                datosLaboralesExists.cargo_id=datosPago.cargo_id,
                datosLaboralesExists.user_id=datosPago.user_id,                
                datosLaboralesExists.tipo_contrato=datosPago.tipo_contrato, 
                datosLaboralesExists.termino_contrato=datosPago.termino_contrato, 
                datosLaboralesExists.fecha_inicio=datosPago.fecha_inicio,                 
                datosLaboralesExists.fecha_fin=datosPago.fecha_fin, 
                datosLaboralesExists.periodo_salario=datosPago.periodo_salario, 
                datosLaboralesExists.modalidad=datosPago.modalidad, 
                datosLaboralesExists.dias_descanso=datosPago.dias_descanso, 
                datosLaboralesExists.salario_base=datosPago.salario_base,
    ...
    def list_history_datos_pago(self,  id:int):
        ...
        nRecord = self.db.query(HistoricoDatosPagoModel).filter(HistoricoDatosPagoModel.datos_laborales_id == id).count()
        ...
        if (nRecord == 0):
            ...
            try:
                ...
                consulta = self.db.query(HistoricoDatosPagoModel).filter(HistoricoDatosPagoModel.datos_laborales_id == id)
