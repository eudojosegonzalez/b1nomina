...
class userController():
    ...
    def list_users(self,page,records, Session):
        '''result= self.db.query(ViewGeneralUser).all().paginate(page=page, per_page=records)
        return result'''
    
        paginacion = Session.query(ViewGeneralUser).paginate(page=page, per_page=records)
        resultados = paginacion.items
        return {
            "pagina": paginacion.page,
            "registros_por_pagina": paginacion.per_page,
            "registros_totales": paginacion.total,
            "registros": resultados,
        }
    
        '''paginacion = self.db.execute(ViewGeneralUser.select().paginate(page=page, per_page=records))
        resultados = paginacion.items
        return {
            "pagina": paginacion.page,
            "registros_por_pagina": paginacion.per_page,
