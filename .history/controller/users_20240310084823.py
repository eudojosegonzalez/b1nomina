...
class userController():
    ...
    def update_user (self, user_updater: int, data : UserSchema , userId):
        try:       
            ...
            paso=1
            nombreOk=ValidationController.validarRut(data.nombres.strip().upper())
            paso=2
            apellidoPaternoOk=ValidationController.validarRut(data.apellido_paterno.strip().upper())
            
            paso=3
            if (len(data.apellido_materno.strip())>0):
                apellidoMaternoOk=ValidationController.validarRut(data.apellido_materno.strip().upper())
            else:
            ...
            if (rutOk and nombreOk and apellidoPaternoOk and apellidoMaternoOk and rutProvisorioOk):
                ...
                if (nRecordUser>0):
                    ...
                    if (nRecordUsername > 0):
                        ...
                        # ojo crear una funcion de usu privado para insertar los datos en el historico
            ...
            return( {"result":"-3","error": f"Error {str(e)} paso {paso}" })
