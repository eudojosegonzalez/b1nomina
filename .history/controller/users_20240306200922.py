    ...
    async def upload_massive_user(self,creatorUserId,file):
        ...
        if (fileExtension in permitedExtensionMassiveUsers):
            ...
            for row in sheet.iter_rows(min_row=2,values_only=True):  # Omitir primera fila (encabezados)
                ...
                arreglo = mi_string.split()
                segundo_elemento = arreglo[0] 

                usernameV = (".".join([primer_elemento, segundo_elemento])).lower()
                ...
                if (result['result']=="1"):
                    ...
                    contacUser=contactUserSchema(
                        
                    )
                    result2=contactUserController.create_contact_user(self, 1 ,contactoUsuario:contactUserSchema)
                ...
                archivoSalida = f"/{ruta}/{uuid.uuid4()}.csv"
