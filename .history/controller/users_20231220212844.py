...
class userController():
    ...
    # @params id: Usuario que efectua la actualización    
    ...
    # @params id: Usuario que efectua la actualización    
    # @params userId: Id del usuario que se actualizará la clave
    # @params password: Nueva clave del usuario que se actualizará
    def update_password_user (self, id:int, userId: int, password : str ):
        try:       
            user = self.db.query(UsuarioModel).filter(UsuarioModel.id==userId).first()
            if (user):
                ...
                self.db.commit()
                # se actualizó la data personal del usuario
                return ({"result":"1","estado":"Password de Usuario Actualizado","UserId":id})
            else:
                # no existe el ID del usuario
                return ({"result":"-1","estado":"Usuario no encontrado","UserId":id})
        except ValueError as e:
