...
class APVController():
    ...
    def create_historico_APVs (self, APV: APVModel, observacion:str):
        ...
        try:
            ...
            newHistoricoAPV= HistoricoAPVModel(
                APV_id = APV.id,
                codigo_previred = APV.codigo_previred,
                nombre = APV.nombre,
                cotizacion=APV.cotizacion,
                cuenta_APV=APV.cuenta_APV,
                sis=APV.sis,
                cuenta_sis_cred=APV.cuenta_sis_cred,
                cuenta_ahorro_APV_cuenta2=APV.cuenta_ahorro_APV_cuenta2,
                codigo_direccion_trabajo=APV.codigo_direccion_trabajo,
                created = APV.created,
                updated = APV.updated,
    ...
    def create_APV(self, APV:APVSchema, userCreatorId:int ):
        ...
        codigo_previred=APV.codigo_previred.upper().strip()
        ...
        nRecordCodigo = self.db.query(APVModel).filter(APVModel.codigo_previred == codigo_previred).count()  
        ...
        if (nRecordNombre > 0):
            ...
            APVExists=self.db.query(APVModel).filter(APVModel.codigo_previred == codigo_previred).first() 
            ...
            try:
                ...
                self.create_historico_APVs(newAPV,"Se creó un APV en el sistema")
                data={
                    "id":newAPV.id,
                    "codigo_previred": newAPV.codigo_previred,
                    "nombre": newAPV.nombre,
                    "cotizacion":newAPV.cotizacion,
                    "cuenta_APV":newAPV.cuenta_APV,
                    "sis":newAPV.sis,
                    "cuenta_sis_cred":newAPV.cuenta_sis_cred,
                    "cuenta_ahorro_APV_cuenta2":newAPV.cuenta_ahorro_APV_cuenta2,
                    "codigo_direccion_trabajo":newAPV.codigo_direccion_trabajo,
    ...
    def search_APV(self,finding ,page, records):
        ...
        try:
            ...
            nRecord=self.db.query(APVModel).filter(APVModel.nombre.like(findingT) | APVModel.nombre.like(findingT)).count()  

            # hay registros
            if (nRecord >0):
                # ejecutamos la consulta
                consulta = self.db.query(APVModel).filter(APVModel.nombre.like(findingT) | APVModel.nombre.like(findingT))
    ...
    def update_APV(self, APV:APVSchema, userUpdaterId:int, id:int):
        ...
        if (nRecord == 0):
            ...
            try:
                ...
                #creamos el registro historico de bancarios del usuario
                self.create_historico_APVs(APVExists ,"Actualización de la data del APV")

                #registramnos los cambios en la tabla de bancarios del usuario
                APVExists.codigo_previred = APV.codigo_previred,
                APVExists.nombre = ((APV.nombre).upper()).strip(),
                APVExists.cotizacion=APV.cotizacion,
                APVExists.cuenta_APV=APV.cuenta_APV,
                APVExists.sis=APV.sis,
                APVExists.cuenta_sis_cred=APV.cuenta_sis_cred,
                APVExists.cuenta_ahorro_APV_cuenta2=APV.cuenta_ahorro_APV_cuenta2,
                APVExists.codigo_direccion_trabajo=APV.codigo_direccion_trabajo,
                ...
                data={
                    "id":APVExists.id,
                    "codigo_previred":APVExists.codigo_previred,
                    "nombre":APVExists.nombre,
                    "cotizacion":APVExists.cotizacion,
                    "cuenta_APV":APVExists.cuenta_APV,  
                    "sis":APVExists.sis,   
                    "cuenta_sis_cred":APVExists.cuenta_sis_cred,
                    "cuenta_ahorro_APV_cuenta2":APVExists.cuenta_ahorro_APV_cuenta2,
                    "codigo_direccion_trabajo":APVExists.codigo_direccion_trabajo,
    ...
    def list_history_APVs(self, page:int, records: int, id:int):
