...
class userController():
    ...
    def create_user(self, usuario:UserSchema):
        ...
        userExistsUserName = self.db.query(UsuarioModel).filter(UsuarioModel.username == userName).count()   
        userExistsUserRut = self.db.query(UsuarioModel).filter(UsuarioModel.rut == rutV).count()
        userExistsUserRutProvisorio=0
        
        if (len(rutProvisorio)>0):
            userExistsUserRutProvisorio = self.db.query(UsuarioModel).filter(UsuarioModel.rut_provisorio == rutProvisorio).count() 
        ...
        if ((userExistsUserRut >0 ) or (userExistsUserRutProvisorio>0)):
            # el rut ya existe
