...
class DashboardAdminController():
    ...
    def get_employee_summary(self):
        nRecordUserActivos=self.db.query(UsuarioModel).filter(UsuarioModel.activo==1).count()

        nRecordUserInactivos=self.db.query(UsuarioModel).filter(UsuarioModel.activo==0).count()
