...
class userController():
    ...
    async def list_users(self,page,records):
        '''result= self.db.query(ViewGeneralUser).all().offset((page - 1) * records).limit(records)
        return result'''
    
        paginacion = await self.db.execute(ViewGeneralUser.select().paginate(page=page, per_page=records))
