...
class sociedadesController():
    ...
    # @params page: pagina de los datos que se mostrará
    # @params records: cantidad de registros por página
    def search_users_sdg(self,id, sociedad_id,sede_id,departamento_id,grupo_id):
        '''
        ...
        try:
            # buscamos si hay resgistros coincidentes
            consulta=self.db.query(ViewGeneralUserSDG).filter(ViewGeneralUserSDGModel.sociedad_id=id).count()


            nRecord=self.db.query(ViewGeneralUserSDG).filter(ViewGeneralUserSDG.sede_id).count()
