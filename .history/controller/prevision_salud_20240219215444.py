...
class PrevisionSaludController():
    ...
    def list_history_prevision_saluds(self, id:int):

        # buscamos si exite la sede
        nRecord = self.db.query(HistoricoPrevisionSaludModel).filter(HistoricoPrevisionSaludModel.prevision_id == id).count()
        ...
        if (nRecord == 0):
            ...
            try:
                # ejecutamos la consulta
                consulta = self.db.query(HistoricoPrevisionSaludModel).filter(HistoricoPrevisionSaludModel.prevision_id == id)
