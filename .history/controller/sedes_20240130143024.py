...
class sedesController():
    ...
    def create_historico_sociedad (self, sociedad: SedeModel, observacion:str):
    ...
    def create_sede(self, sede:SedeSchema, userCreatorId:int ):
        ...
        nombreSociedad=sede.nombre.upper().strip()
        rutSociedad=sede.rut.upper().strip()


        # contamos si existe una sociedad con el mismo nombre
        nRecordNombre = self.db.query(SedeModel).filter(SedeModel.nombre == nombreSociedad).count()  

        # contamos si existe una sociedad con el mismo rut
        nRecordRut = self.db.query(SedeModel).filter(SedeModel.rut == rutSociedad).count()         


        if (nRecordNombre > 0):
            # buscamos la sociedad con el nombre y lo devolvemos
            sociedadExists=self.db.query(SedeModel).filter(SedeModel.nombre == nombreSociedad).first() 

            # devolvemos la sociedad que ya existe
            return ({"result":"-1","estado":"Existe una sociedad con ese nombre","data":sociedadExists})
        elif (nRecordRut > 0):
            # buscamos la sociedad con el nombre y lo devolvemos
            sociedadExists=self.db.query(SedeModel).filter(SedeModel.rut == rutSociedad).first() 
            ...
            try:
                ...
                self.create_historico_sede(newSede,"Se creó una sociedad en el sistema")
    ...
    def get_sede(self,id:int):
        ...
        # buscamos si este eciste esta sede
        ...
        if (nRecord == 0):
            ...
            try:
                sociedadExits = self.db.query(SedeModel).filter(SedeModel.id == id).first()                  
    ...
    def search_sociedades(self,finding ,page, records):
        ...
        try:
            ...
            nRecord=self.db.query(SedeModel).filter(SedeModel.nombre.like(findingT) | SedeModel.rut.like(findingT)).count()  

            # hay registros
            if (nRecord >0):
                # ejecutamos la consulta
                consulta = self.db.query(SedeModel).filter(SedeModel.nombre.like(findingT) | SedeModel.rut.like(findingT))
    ...
    def update_sede(self, sede:SedeSchema, userUpdaterId:int, id:int):
        ...
        if (nRecord == 0):
            ...
            try:
                ...
                sociedadExists = self.db.query(SedeModel).filter(SedeModel.id == id).first()             

                #creamos el registro historicosociedades 
                self.create_historico_sede(sociedadExists ,"Actualización de la data de la sociedad")

                #registramnos los cambios en la tabla de bancarios del usuario
                sociedadExists.nombre=((sede.nombre).upper()).strip(),
                sociedadExists.rut=((sede.rut).upper()).strip(),
                sociedadExists.region_id=sede.region_id,
                sociedadExists.comuna_id=sede.comuna_id,
                sociedadExists.direccion=((sede.direccion).upper()).strip(),
                sociedadExists.ciudad=((sede.ciudad).upper()).strip(),
                sociedadExists.icono=sede.icono,
