        ...
        try:       
            ...
            if (user):
                ...
                #actualizamos la data personal del usuario
                    ...
                    #actualizamos la data personal del usuario
                    #veiricamos que ese usuario no este siendo usado por otro
                    nRecordUsername=self.db.query(UsuarioModel).filter(UsuarioModel.username==data.username and UsuarioModel.id!=data.id).count()

                    #veiricamos que ese rut no este registrado en el sistema
                    nRecordRut=self.db.query(UsuarioModel).filter(UsuarioModel.rut==data.rut and UsuarioModel.id!=data.id).count() 

                    #veiricamos que ese rut_provisorio no este registrado en el sistema
                    nRecordRutProvisorio=self.db.query(UsuarioModel).filter(UsuarioModel.rut_provisorio==data.rut_provisorio and UsuarioModel.id!=data.id).count()                                       

                    if (nRecordUsername > 0):
                        # se el rut ya esta siendo usado por otra persona
                        #buscamos el id de este dato que existe 
                        userExists=self.db.query(UsuarioModel).filter(UsuarioModel.username==data.username and UsuarioModel.id!=data.id).first()
                        return ({"result":"-2","estado":"Este Username ya esta siendo ocupado en el sistema","UserId": userExists.id})     
                    elif (nRecordRut > 0):    
                        # se el rut ya esta siendo usado por otra persona
                        #buscamos el id de este dato que existe 
                        userExists=self.db.query(UsuarioModel).filter(UsuarioModel.rut==data.rut and UsuarioModel.id!=data.id).first()
                        ...
                        # se el rut ya esta siendo usado por otra persona
                        #buscamos el id de este dato que existe 
                        userExists=self.db.query(UsuarioModel).filter(UsuarioModel.rut_provisorio==data.rut_provisorio and UsuarioModel.id!=data.id).first()
