...
def list_prevision(page : int = 1, records : int = 20)->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = PrevisionSaludController(db).list_prevision_salud(page,records)
...
# ruta para listar las previsions en el sistema segun la sociedad que las agrupa
@prevision_salud_router.get ('/list_prevision_sociedad', 
tags=["Prevision Salud"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
...
def list_prevision_sociedad(idSociedad: int, page : int = 1, records : int = 20)->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = PrevisionSaludController(db).list_previsions_sociedad(idSociedad,page,records)
...
responses=
    { 
        200: {
                "description": "prevision Encontrada",
                "content": { 
                    "application/json":
                        { 
                            "example":
                                {
                                    ...
                                    "data": "{'sociedad_id': '1','id': '1','region_id': '1','direccion': 'DIRECCION prevision DEMO','ciudad': 'DEMO CIUDAD','updated': '2024-02-01T13:14:54','updater_user': '1','nombre': 'prevision UNO',   'comuna_id': '1101','created': '2024-02-01T13:13:35','creator_user': '1'}",
                                }
                        } 
...
def get_prevision(id: int):
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = PrevisionSaludController(db).get_prevision(id)
    # debemnos convertir los objetos tipo BD a Json
    if (result):
        if (result["result"]=="1"):
            data=result['data']
...
def list_history_prevision(page : int = 1, records : int = 20, id : int =Path(ge=1, lt=1000))->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = PrevisionSaludController(db).list_history_previsions(page,records,id)
...
def search_prevision(finding : str = Query (min_length=os.getenv("MIN_STR_SEARCH_USER"), max_length=os.getenv("MAX_STR_SEARCH_USER")), page : int = 1, records : int = 20)->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = PrevisionSaludController(db).search_prevision_salud(finding,page,records)
