...
def update_preuser2(preUser:PreUserSchema, user_updater: int = Query(ge=1, le=os.getenv('MAX_ID_USERS')), idUser : int = Path(ge=1, le=os.getenv('MAX_ID_USERS')) )->dict:
    db = Session()
    # buscamos el registro
    result = userController(db).update_preuser2(idUser,user_updater,preUser) 
    if (result['result']=="1"):
        return JSONResponse(status_code=200,content={"message":"Usuario actualizado"})    
    elif (result['result']=="-1"):
        return JSONResponse(status_code=404,content={"message":"Usuario no encontrado"}) 
