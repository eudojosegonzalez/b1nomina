...
async def list_user(page : int = 1, records : int = 20):
    db = Session()
...
def update_user(userId: int, usuario:User):
    db = Session()
...
def update_password_user(id:int, userId: int, passsword:str):
    db = Session()
    # buscamos el registro
    result = userController(db).update_user(id,userId, passsword) 
    if (result['result']=="1"):
...
# Funcion para csubir archivos al profile de un usuario
@user_router.post ('/upload_file_user',
tags=['Usuarios'],
dependencies=[Depends(JWTBearer())], 
responses=
    { 
        201: {
            "description": "Se subió un archivo de un usuario al sistema",
            "content": { 
                "application/json":{
                    "example":
                        {
                            "message":"Se subió un archivo de un usuario al sistema",
                            "newUserId":"1"
                        }
                    } 
                }       
