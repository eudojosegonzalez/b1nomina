...
@categorias_configuracion_router.post ('/create_configuracion', 
tags=["Configuracion"],
...
def create_configuracion(categorias_configuracion:CategoriasConfiguracionSchema, userCreatorId : int = Query (ge=1, le=os.getenv("MAX_ID_USERS")))->dict:
    db = Session()
    result=ConfiguracionController(db).create_categoria_configuracion(categorias_configuracion,userCreatorId)
...
@categorias_configuracion_router.get ('/list_configuracion', 
tags=["Configuracion"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
...
@categorias_configuracion_router.get ('/list_configuracion_sociedad', 
tags=["Configuracion"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
...
def get_configuracion(id: int):
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = ConfiguracionController(db).get_categoria_configuracion(id)
    # debemnos convertir los objetos tipo BD a Json
    if (result):
        if (result["result"]=="1"):
            data=result['data']
...
@categorias_configuracion_router.get ('/configuracion/{id}/list_historico', 
tags=["Configuracion"],
...
def list_history_configuracion( id : int =Path(ge=1, lt=1000))->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = ConfiguracionController(db).list_history_categoria_configuracion(id)
...
@categorias_configuracion_router.put ('/configuracion/{id}/update', 
tags=["Configuracion"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
...
def update_configuracion(categorias_configuracion:CategoriasConfiguracionSchema, user_updater: int = Query(ge=1, le=os.getenv('MAX_ID_USERS')), id : int = Path(ge=1,le=1000))->dict:
    db = Session()
    # buscamos el registro
    result = ConfiguracionController(db).update_configuracion(categorias_configuracion, user_updater, id) 
    if (result['result']=="1"):
        data=result['data']
