...
@pic_user_router.post ('/user/{id}/upload_file_users',
...
async def file_upload_user(creatorUserId : int = Query (ge=1, lt=os.getenv("MAX_ID_USERS")) ,id : int = Path (ge=1, lt=os.getenv("MAX_ID_USERS")),file : UploadFile=File())->dict:
    db = Session()
    result=PicUserController(db).upload_file_user(creatorUserId,id,file)
...
def get_file_user(id:int = Path(ge=1, le=os.getenv("MAX_FILES_USERS")))->dict:
    ...
    result = PicUserController(db).get_file_user(id)
...
@pic_user_router.delete ('/user/delete_file_users',
tags=['Fotos de Usuarios'],
#dependencies=[Depends(JWTBearer())], 
responses=
    { 
