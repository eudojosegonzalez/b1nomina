...
def create_banco(banco:bancosSchema)->dict:
    db = Session()
    result=bancosController(db).create_banco(banco)
    # evaluamos el resultado
    estado=result['result']

    if (estado=="1") :
        # se inserto el registro sin problemas
        newBancoId=result["newBancoId"]
        return JSONResponse (status_code=201,content={"message":"Se creo un Banco en el sistema","newUserId":newBancoId})     
    else:
