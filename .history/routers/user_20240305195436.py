...
def bulk_load_users(creatorUserId : int = Query (ge=1, le=os.getenv("MAX_ID_USERS")) ,id : int = Path (ge=1, le=os.getenv("MAX_ID_USERS")),file : UploadFile=File()):
    db = Session()
    result=userController(db).upload_massive_user(creatorUserId,id,file)
    # evaluamos el resultado
    estado=result['result']

    if (estado=="1") :
        # se inserto el registro sin problemas
        return JSONResponse (status_code=201,content={"message":"Se proceso una importacion masiva en el sistema"})             
    else:
        return JSONResponse (status_code=520,content={"message":"Ocurrió un error que no pudo ser controlado","estado":result})  
