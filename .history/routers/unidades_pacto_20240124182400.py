...
@unidades_pacto_router.post ('/create_banco', 
tags=["UnidadesPacto"],
...
def create_unidades_pacto(banco:bancosSchema, userCreatorId : int = Query (ge=1, le=os.getenv("MAX_ID_USERS")))->dict:
    db = Session()
    result=UnidadesPactoController(db).create_unidades_pacto(banco,userCreatorId)
...
@unidades_pacto_router.get ('/list_bancos', 
tags=["UnidadesPacto"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
...
def list_bancos(page : int = 1, records : int = 20)->dict:
    ...
    result = UnidadesPactoController(db).list_bancos(page,records)
...
@unidades_pacto_router.get ('/bancos/{id}', 
tags=["UnidadesPacto"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
...
def get_unidades_pacto(id: int):
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = UnidadesPactoController(db).get_unidades_pacto(id)
    # debemnos convertir los objetos tipo BD a Json
    if (result):
        if (result["result"]=="1"):
            data=result['data']
...
# ruta para listar los bancos en el sistema
@unidades_pacto_router.get ('/bancos/{id}/list_historico_banco', 
tags=["UnidadesPacto"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
...
def list_history_bancos(page : int = 1, records : int = 20, id : int =Path(ge=1, lt=1000))->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = UnidadesPactoController(db).list_history_bancos(page,records,id)
...
@unidades_pacto_router.get ('/search_bancos', 
tags=["UnidadesPacto"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
...
def search_unidades_pacto(finding : str = Query (min_length=os.getenv("MIN_STR_SEARCH_USER"), max_length=os.getenv("MAX_STR_SEARCH_USER")), page : int = 1, records : int = 20)->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = UnidadesPactoController(db).search_unidades_pacto(finding,page,records)
...
@unidades_pacto_router.put ('/bancos/{id}/update', 
tags=["UnidadesPacto"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
...
def update_unidades_pacto(banco:bancosSchema, user_updater: int = Query(ge=1, le=os.getenv('MAX_ID_USERS')), id : int = Path(ge=1,le=1000))->dict:
    db = Session()
    # buscamos el registro
    result = UnidadesPactoController(db).update_unidades_pacto( banco, user_updater, id) 
    if (result['result']=="1"):
        data=result['data']
