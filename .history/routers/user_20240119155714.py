...
def update_user(usuario:User, user_updater: int = Query(ge=1, le=os.getenv('MAX_ID_USERS')))->dict:
    ...
    if (result['result']=="1"):
        ...
        return JSONResponse(status_code=521,content={"message":"Este Username esta siendo usado por otro usuario"})     
    elif (result['result']=="-4"):
        return JSONResponse(status_code=522,content={"message":"Este RUT está registrado a nombre de otro usuario"})       
    elif (result['result']=="-5"):
        return JSONResponse(status_code=522,content={"message":"Este RUT  provisorio está registrado a nombre de otro usuario"})       
    else:
