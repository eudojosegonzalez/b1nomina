...
def get_bancarios_user(id: int = Query (ge=1, le= Query(ge=1, le=os.getenv("MAX_ID_USERS")))):
    db = Session()
    # almacenamos el listado de usarios en un resultset
...
async  def get_historico_bancarios_user(page: int = Query (ge=1, le= Query(ge=1, le=os.getenv("MAX_ID_USERS"))))->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = bancariosUserController(db).list_bancarios_users()
