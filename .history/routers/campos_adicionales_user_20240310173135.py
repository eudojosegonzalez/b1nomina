...
def create_campos_adicionales_user(camposUser:CamposAdicionalesUserSchema, userCreatorId : int = Query (ge=1, le=os.getenv("MAX_ID_USERS")))->dict:
    ...
    if (estado=="1") :
        ...
        return JSONResponse (status_code=201,content={"message":"Se creo una sede en el sistema","sede":jsonable_encoder(newCamposuser)})     
    elif  (estado=="-1"):
        # el username ya existe no puede volver a insertarlo
        return JSONResponse (status_code=521,content={"message":"Y existe un registro de campos adicionales para este usuario","Campos Adicionales usuario":jsonable_encoder(result['data'])})     
    elif (estado=="-2"):
