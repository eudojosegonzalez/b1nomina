...
from schemas.categorias_configuracion import CategoriasConfiguracion
...
def create_categorias_configuracion(categorias_configuracion:categorias_configuracionSchema, userCreatorId : int = Query (ge=1, le=os.getenv("MAX_ID_USERS")))->dict:
    db = Session()
...
def list_categorias_configuracion(page : int = 1, records : int = 20)->dict:
    ...
    result = CategoriasConfiguracionsController(db).list_categorias_configuracions(page,records)
...
def update_categorias_configuracion(categorias_configuracion:categorias_configuracionSchema, user_updater: int = Query(ge=1, le=os.getenv('MAX_ID_USERS')), id : int = Path(ge=1,le=1000))->dict:
    db = Session()
