...
def create_mutuales(mutuales:mutualesSchema, userCreatorId : int = Query (ge=1, le=os.getenv("MAX_ID_USERS")))->dict:
    ...
    if (estado=="1") :
        ...
        return JSONResponse (status_code=521,content={"message":"existe un mutuales con este nombre, no puede volver a crearlo","mutuales":jsonable_encoder(result['data'])})     
    elif (estado=="-2"):
        return JSONResponse (status_code=521,content={"message":"existe un mutuales con este codigo, no puede volver a crearlo","mutuales":jsonable_encoder(result['data'])})     
...
# ruta para listar los mutuales en el sistema
@mutuales_router.get ('/mutuales/{id}/list_historico_mutuales', 
tags=["mutuales"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
...
# ruta para buscar un mutuales por nombre o codigo
