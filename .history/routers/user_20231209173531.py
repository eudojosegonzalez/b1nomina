    ...
    if (userVerified):
        ...
        if (autorized):
            ...
            if (userActive):
                ...
                # retornamos el token del usuario y su ID
                expiration_time = jwt.get_expiration_time(token)                
                return JSONResponse (status_code=202,content={"token":token,"userId":userVerified.id,'expiration_time':expiration_time})
            else:
                #usuario suspendido
