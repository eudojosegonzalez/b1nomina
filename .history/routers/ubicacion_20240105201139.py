...
@user_ubicacion_router.get ('/user_history_ubication/{id}',
tags=["Contacto"], 
#dependencies=[Depends(JWTBearer())],
responses=
    { 
        201: {
            ...
            "content": { 
                "application/json":{
                    "example":
                        {
                            "message":"Se consiguieron los datos historicos de ubicacion del usuario",
...
def list_history_ubication_user(id: int = Path(ge=1, le=os.getenv("MAX_ID_USERS")))->dict:
    db = Session()
    result=ubicationUserController(db).list_history_ubication_user(id)
    # evaluamos el resultado
    estado=result['result']

    if (estado=="1") :
        # se consiguieron los datos históricos de ubiucacion del usuario
        data=result["listHistoryUbication"]
        return JSONResponse (status_code=201,content=jsonable_encoder(data))     
    elif (estado=="-2"):
        # no se consiguieron los datos de contacto del cliente
        return JSONResponse (status_code=404,content={"message":"No se consiguieron los datos historicos de ubicación del usuario","estado":result}) 
    else:     
        # ocurrió un error a nivel de servidor
        return JSONResponse (status_code=520,content={"message":"Ocurrió un error que no pudo ser controlado","estado":estado}) 
