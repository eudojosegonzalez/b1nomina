...
from models.regiones import Regiones
...
def create_user_ubicacion(userCreatorId:int,ubicacionUserSchema:UbicacionUserSchema)->dict:
    ...
    if (estado=="1") :
        ...
        return JSONResponse (status_code=500,content={"message":"Ya existen los datos de contacto de este usuario, no puede volver a crearlos","ubicationUserId":result})    
    else:
...
def get_user_contact(id: int)->dict:
    db = Session()
    result=ubicationUserController(db).get_contact_user(id)
    # evaluamos el resultado
    estado=result['result']
