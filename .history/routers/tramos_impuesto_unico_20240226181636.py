...
@tramos_impuesto_unico_router.get ('/list_tramo_impuesto_unico_sociedad', 
tags=["Tramos Impuesto Unico"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
...
def list_tramo_impuesto_unico_sociedad(idSociedad: int, page : int = 1, records : int = 20)->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = TramosImpuestoUnicoController(db).list_tramo_impuesto_unico_sociedad(idSociedad,page,records)
...
@tramos_impuesto_unico_router.get ('/sede/{id}', 
tags=["Tramos Impuesto Unico"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
...
@tramos_impuesto_unico_router.get ('/sede/{id}/list_historico', 
tags=["Tramos Impuesto Unico"],
...
def list_history_tramo_impuesto_unico(page : int = 1, records : int = 20, id : int =Path(ge=1, lt=1000))->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = TramosImpuestoUnicoController(db).list_history_tramo_impuesto_unico(page,records,id)
...
def search_tramo_impuesto_unico(finding : str = Query (min_length=os.getenv("MIN_STR_SEARCH_USER"), max_length=os.getenv("MAX_STR_SEARCH_USER")), page : int = 1, records : int = 20)->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = TramosImpuestoUnicoController(db).search_tramo_impuesto_unico(finding,page,records)
...
@tramos_impuesto_unico_router.put ('/sede/{id}/update', 
tags=["Tramos Impuesto Unico"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
...
def update_tramo_impuesto_unico(tramosImpuestoUnico:TramosImpuestoUnicoSchema, user_updater: int = Query(ge=1, le=os.getenv('MAX_ID_USERS')), id : int = Path(ge=1,le=1000))->dict:
    ...
    result = TramosImpuestoUnicoController(db).update_tramo_impuesto_unico(sede, user_updater, id) 
    if (result['result']=="1"):
        data=result['data']
