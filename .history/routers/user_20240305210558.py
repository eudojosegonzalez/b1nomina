...
from schemas.user2 import UserMassive
...
def bulk_load_users(creatorUserId : int = Query (ge=1, le=os.getenv("MAX_ID_USERS")),file : UploadFile=File()):
    db = Session()
    result=userController(db).upload_massive_user(creatorUserId,file)
    # evaluamos el resultado
    estado=result['result']
