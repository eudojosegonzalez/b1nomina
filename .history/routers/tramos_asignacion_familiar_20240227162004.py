...
from controller.tramos_asignacion_familiar import TramosAsignacionFamiliarController
...
def create_tramo_asignacion_familiar(tramosAsignacionFamiliar:TramosAsignacionFamiliarSchema, userCreatorId : int = Query (ge=1, le=os.getenv("MAX_ID_USERS")))->dict:
    db = Session()
    result=TramosAsignacionFamiliarController(db).create_tramo_asignacion_familiar(tramosAsignacionFamiliar,userCreatorId)
...
def list_tramo_asignacion_familiar()->dict:
    ...
    result = TramosAsignacionFamiliarController(db).list_tramo_asignacion_familiar()
...
@tramos_asignacion_familiar_router.get ('/tramo_impuesto_unico/{id}', 
tags=["Tramos Asignacion Familiar"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
...
def get_tramo_asignacion_familiar(id: int):
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = TramosAsignacionFamiliarController(db).get_tramo_asignacion_familiar(id)
    # debemnos convertir los objetos tipo BD a Json
    if (result):
        if (result["result"]=="1"):
            data=result['data']
...
@tramos_asignacion_familiar_router.get ('/tramo_impuesto_unico/{id}/list_historico', 
tags=["Tramos Asignacion Familiar"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
...
def list_history_tramo_asignacion_familiar( id : int =Path(ge=1, lt=1000))->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = TramosAsignacionFamiliarController(db).list_history_tramo_asignacion_familiar(id)
...
def search_tramo_asignacion_familiar(finding : str = Query (min_length=os.getenv("MIN_STR_SEARCH_USER"), max_length=os.getenv("MAX_STR_SEARCH_USER")))->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = TramosAsignacionFamiliarController(db).search_tramo_asignacion_familiar(finding)
...
@tramos_asignacion_familiar_router.put ('/tramo_impuesto_unico/{id}/update', 
tags=["Tramos Asignacion Familiar"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
...
def update_tramo_asignacion_familiar(tramosAsignacionFamiliar:TramosAsignacionFamiliarSchema, user_updater: int = Query(ge=1, le=os.getenv('MAX_ID_USERS')), id : int = Path(ge=1,le=1000))->dict:
    ...
    result = TramosAsignacionFamiliarController(db).update_tramo_asignacion_familiar(tramosImpuestoUnico, user_updater, id) 
    if (result['result']=="1"):
        data=result['data']
