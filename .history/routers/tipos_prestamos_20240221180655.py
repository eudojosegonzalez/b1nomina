...
@sedes_router.post ('/create_tipos_prestamos', 
...
def create_tipos_prestamos(sede:SedeSchema, userCreatorId : int = Query (ge=1, le=os.getenv("MAX_ID_USERS")))->dict:
    db = Session()
    result=sedesController(db).create_tipos_prestamos(sede,userCreatorId)
...
@sedes_router.get ('/list_tipos_prestamos', 
tags=["Tipos Prestamos"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
...
def list_tipos_prestamos(page : int = 1, records : int = 20)->dict:
    ...
    result = sedesController(db).list_tipos_prestamos(page,records)
...
@sedes_router.get ('/list_tipos_prestamos_sociedad', 
tags=["Tipos Prestamos"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
...
def list_tipos_prestamos_sociedad(idSociedad: int, page : int = 1, records : int = 20)->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = sedesController(db).list_tipos_prestamoss_sociedad(idSociedad,page,records)
...
def get_tipos_prestamos(id: int):
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = sedesController(db).get_tipos_prestamos(id)
    # debemnos convertir los objetos tipo BD a Json
    if (result):
        if (result["result"]=="1"):
            data=result['data']
...
def list_history_tipos_prestamos(page : int = 1, records : int = 20, id : int =Path(ge=1, lt=1000))->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = sedesController(db).list_history_tipos_prestamoss(page,records,id)
...
@sedes_router.get ('/search_tipos_prestamos', 
tags=["Tipos Prestamos"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
...
def search_tipos_prestamos(finding : str = Query (min_length=os.getenv("MIN_STR_SEARCH_USER"), max_length=os.getenv("MAX_STR_SEARCH_USER")), page : int = 1, records : int = 20)->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = sedesController(db).search_tipos_prestamoss(finding,page,records)
...
def update_tipos_prestamos(sede:SedeSchema, user_updater: int = Query(ge=1, le=os.getenv('MAX_ID_USERS')), id : int = Path(ge=1,le=1000))->dict:
    ...
    result = sedesController(db).update_tipos_prestamos(sede, user_updater, id) 
    if (result['result']=="1"):
        data=result['data']
