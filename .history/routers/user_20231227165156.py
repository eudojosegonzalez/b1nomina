...
async def file_upload_user(creatorUserId : int = Query (ge=1, lt=20000) ,id : int = Path (ge=1, lt=20000),file : UploadFile=File())->dict:
    ...
    if (estado=="1") :
        ...
        return JSONResponse (status_code=501,content={"message":"Tipo de archivo no permitido","estado":result})    
    elif  (estado=="-2"):
        return JSONResponse (status_code=502,content={"message":"El archivo es demasiado grande","estado":result})        
    else:
        return JSONResponse (status_code=503,content={"message":"Ocurrió un error que no pudo ser controlado","estado":result})
