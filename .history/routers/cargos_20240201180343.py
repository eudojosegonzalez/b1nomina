...
from controller.cargos import cargossController
...
cargoss_router = APIRouter(prefix="/V1.0")

# -------- Rutas Bancos ------------
# ruta para crear las cargoss
@cargoss_router.post ('/create_cargos', 
tags=["cargoss"],
...
def create_cargos(cargos:cargosSchema, userCreatorId : int = Query (ge=1, le=os.getenv("MAX_ID_USERS")))->dict:
    db = Session()
    result=cargossController(db).create_cargos(cargos,userCreatorId)
    # evaluamos el resultado
    estado=result['result']

    if (estado=="1") :
        # se inserto el registro sin problemas
        newcargos=result['data']
        return JSONResponse (status_code=201,content={"message":"Se creo una cargos en el sistema","cargos":jsonable_encoder(newcargos)})     
    elif  (estado=="-1"):
        # el username ya existe no puede volver a insertarlo
        return JSONResponse (status_code=521,content={"message":"existe una cargos con este nombre, no puede volver a crearla","cargos":jsonable_encoder(result['data'])})     
    elif (estado=="-2"):
        return JSONResponse (status_code=521,content={"message":"existe una cargos con este rut, no puede volver a crearla","cargos":jsonable_encoder(result['data'])})     

    else:
        return JSONResponse (status_code=520,content={"message":"Ocurrió un error que no pudo ser controlado","estado":result})    
             

# ruta para listar las cargoss en el sistema
@cargoss_router.get ('/list_cargos', 
tags=["cargoss"],
dependencies=[Depends(JWTBearer())],
responses=
    { 
...
def list_cargos(page : int = 1, records : int = 20)->dict:
    ...
    result = cargossController(db).list_cargoss(page,records)
...
def get_cargos(id: int):
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = cargossController(db).get_cargos(id)
    # debemnos convertir los objetos tipo BD a Json
    if (result):
        if (result["result"]=="1"):
            data=result['data']
            return JSONResponse(status_code=200,content={"cargos":jsonable_encoder(data)})    
        else:
...
# ruta para listar los datos historicos de la cargoss por ID
@cargoss_router.get ('/cargos/{id}/list_historico', 
tags=["cargoss"],
dependencies=[Depends(JWTBearer())],
responses=
    { 
...
def list_history_cargos(page : int = 1, records : int = 20, id : int =Path(ge=1, lt=1000))->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = cargossController(db).list_history_cargoss(page,records,id)
...
# ruta para buscar una cargos por nombre o rut
@cargoss_router.get ('/search_cargos', 
tags=["cargoss"],
dependencies=[Depends(JWTBearer())],
responses=
    { 
...
def search_cargos(finding : str = Query (min_length=os.getenv("MIN_STR_SEARCH_USER"), max_length=os.getenv("MAX_STR_SEARCH_USER")), page : int = 1, records : int = 20)->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = cargossController(db).search_cargoss(finding,page,records)
...
@cargoss_router.put ('/cargos/{id}/update', 
tags=["cargoss"],
dependencies=[Depends(JWTBearer())],
responses=
    { 
...
def update_cargos(cargos:cargosSchema, user_updater: int = Query(ge=1, le=os.getenv('MAX_ID_USERS')), id : int = Path(ge=1,le=1000))->dict:
    db = Session()
    # buscamos el registro
    result = cargossController(db).update_cargos(cargos, user_updater, id) 
    if (result['result']=="1"):
        data=result['data']
        return JSONResponse(status_code=200,content={"message":"cargos actualizada","cargos":jsonable_encoder(data)})    
    elif (result['result']=="-1"):
