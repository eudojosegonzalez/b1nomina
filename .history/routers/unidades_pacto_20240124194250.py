...
def get_unidades_pacto(id: int):
    ...
    if (result):
        if (result["result"]=="1"):
            data=result['data']
            return JSONResponse(status_code=200,content={"Unidad de Pacto":jsonable_encoder(data)})    
        else:
...
# ruta para listar los bancos en el sistema
@unidades_pacto_router.get ('/unidad_pacto/{id}/list_historico_unidad_pacto', 
tags=["UnidadesPacto"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
