...
async def bulk_load_users(creatorUserId : int = Query (ge=1, le=os.getenv("MAX_ID_USERS")),file : UploadFile=File()):
    db = Session()
    result = await userController(db).upload_massive_user(creatorUserId,file)
    # evaluamos el resultado
    estado=result['result']
    ...
    if (estado=="1") :
        # se inserto el registro sin problemas
        newUserId=result['newUserId']
        return JSONResponse (status_code=201,content={"message":"Se proceso una importacion masiva en el sistema","newUserId":newUserId}) 
    elif (estado=="-2"):
