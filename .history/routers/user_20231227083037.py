...
async def list_users(page : int = 1, records : int = 20):
    db = Session()
...
@user_router.post ('/user/upload_file_users',
tags=['Usuarios'],
dependencies=[Depends(JWTBearer())], 
responses=
    { 
...
@user_router.get (
    ...
    responses=
        { 
            ...
            404: {
                "description": "La búsquea no arrojó resultados",
                "content": { 
                    "application/json":{ 
                        "example":
                            {
                                "message":"La búsquea no arrojó resultados"
                            }
                        } 
                    }       
                },   
...
def search_user(finding:str = Path(min_length=3, max_length=250)):
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = userController(db).search_user(finding)
    # debemnos convertir los objetos tipo BD a Json
    if (result):
        if (result["result"]=="1"):
            data=result['resultado']
