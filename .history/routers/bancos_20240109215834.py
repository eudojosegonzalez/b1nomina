...
def update_banco(banco:BancosSchema, user_updater: int = Query(ge=1, le=os.getenv('MAX_ID_USERS')))->dict:
    db = Session()
    # buscamos el registro
    result = bancosController(db).update_banco(user_updater,banco) 
    if (result['result']=="1"):
        data=result['data']
