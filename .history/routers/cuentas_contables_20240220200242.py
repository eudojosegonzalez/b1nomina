...
def create_cuenta_contable(cuentaContable:CuentasContablesSchema, userCreatorId : int = Query (ge=1, le=os.getenv("MAX_ID_USERS")))->dict:
    db = Session()
    result=CuentasContablesController(db).create_cuenta_contable(prevision,userCreatorId)
...
@cuentas_contables_router.get ('/cuentas_contablez/{id}', 
tags=["Cuentas Contables"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
        200: {
                "description": "Cuentas Contables encontrada",
                "content": { 
                    "application/json":
                        { 
                            "example":
                                {
                                    "message":"Cuentas Contables encontrada",
...
@cuentas_contables_router.get ('/cuentas_contablez/{id}/list_historico', 
tags=["Cuentas Contables"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
...
@cuentas_contables_router.put ('/cuentas_contablez/{id}/update', 
tags=["Cuentas Contables"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
