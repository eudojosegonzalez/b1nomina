...
async def file_upload_user(creatorUserId : int = Query (ge=1, lt=20000) ,id : int = Path (ge=1, lt=20000),file : UploadFile=File())->dict:
    ...
    if (estado=="1") :
        ...
        return JSONResponse (status_code=500,content={"message":"El archivo es demasiado grande","estado":result})        
    else:
        return JSONResponse (status_code=500,content={"message":"Ocurrió un error que no pudo ser controlado","estado":result})
...
async def search_users(finding : str = Query (min_length=3, max_length=100), page : int = 1, records : int = 20)->dict:
    ...
    if (result):
        if (result["result"]=="1"):
            ...
            return JSONResponse(status_code=500,content={"message":"System Error","error":result})          
    else:
        return JSONResponse(status_code=500,content={"message":"System Error","error":result})          
