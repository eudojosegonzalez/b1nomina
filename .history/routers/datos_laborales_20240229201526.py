...
def create_datos_laborales(datosLaborales:DatosLaboralesSchema, userCreatorId : int = Query (ge=1, le=os.getenv("MAX_ID_USERS")))->dict:
    ...
    if (estado=="1"):
        ...
        return JSONResponse (status_code=201,content={"message":"Se creo un Dato Laboral en el sistema","DatoLaboral":jsonable_encoder(newDatoLaboral)})     
    elif  (estado=="-1"):
        # el username ya existe no puede volver a insertarlo
        datoLaboral=result['data']
        return JSONResponse (status_code=521,content={"message":"Este usuario ya tiene Datos Laborales no puede volver a crearlo","DatoLaboral":jsonable_encoder(datoLaboral)})     
