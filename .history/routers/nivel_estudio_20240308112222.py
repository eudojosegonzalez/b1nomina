...
from schemas.nivel_estudio import NivelEstudio
...
def create_nivel_estudio(sede:SedeSchema, userCreatorId : int = Query (ge=1, le=os.getenv("MAX_ID_USERS")))->dict:
    db = Session()
    result=NivelEstudioController(db).create_nivel_estudio(sede,userCreatorId)
...
def list_nivel_estudio(page : int = 1, records : int = 20)->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = NivelEstudioController(db).list_nivel_estudios(page,records)
...
def update_nivel_estudio(sede:SedeSchema, user_updater: int = Query(ge=1, le=os.getenv('MAX_ID_USERS')), id : int = Path(ge=1,le=1000))->dict:
    db = Session()
    # buscamos el registro
    result = NivelEstudioController(db).update_nivel_estudio(sede, user_updater, id) 
    if (result['result']=="1"):
        data=result['data']
