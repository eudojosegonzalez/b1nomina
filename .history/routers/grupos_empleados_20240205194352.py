...
@grupos_empleados_router.post ('/create_grupo_empĺeado', 
tags=["GruposEmpleados"],
...
@grupos_empleados_router.get ('/list_sociedad', 
...
def list_grupo_empleados(page : int = 1, records : int = 20)->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = GrupoEmpleadosController(db).list_sociedades(page,records)
...
@grupos_empleados_router.get ('/grupo_empleados/{id}/list_sede', 
tags=["Grupos Empleados"],
#dependencies=[Depends(JWTBearer())],
responses=
    { 
        200: {
                "description": "Grupo Empleados",
                "content": { 
                    "application/json":
                        { 
                            "example":
                                {
                                    "message":"Grupo Empleados encontrado",
...
def search_grupo_empleados(finding : str = Query (min_length=os.getenv("MIN_STR_SEARCH_USER"), max_length=os.getenv("MAX_STR_SEARCH_USER")), page : int = 1, records : int = 20)->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = GrupoEmpleadosController(db).search_sociedades(finding,page,records)
