...
def update_user(usuario:User, user_updater: int = Query(ge=1, le=os.getenv('MAX_ID_USERS')),id : int =Path(ge=1, le=os.getenv('MAX_ID_USERS')))->dict:
    ...
    if (result['result']=="1"):
        ...
        return JSONResponse(status_code=521,content={"message":f"Este Username esta siendo usado por otro usuario userId={result['UserId']}, por favor rectique los datos"})     
    elif (result['result']=="-4"):
        return JSONResponse(status_code=522,content={"message":f"Este RUT está registrado a nombre de otro usuario userId={result['UserId']}, por favor rectique los datos"})       
    elif (result['result']=="-5"):
        return JSONResponse(status_code=522,content={"message":f"Este RUT  provisorio está registrado a nombre de otro usuario userId={result['UserId']}, por favor rectique los datos"})       
