...
async def list_user(page: int = 1, per_page: int = 10, session: Session = Depends(Session)):
        paginacion = await session.query(ViewGeneralUser).paginate(page=page, per_page=per_page)
...
''' async def list_user(page: int = 1, records: int = 10)->list[User]:
    db = Session()
