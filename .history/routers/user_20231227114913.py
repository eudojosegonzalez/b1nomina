...
Created: 2023-12
'''
from fastapi import APIRouter,Body,File,Path,Query, Depends
...
async def file_upload_user(id = Path (ge=1, lt=20000),file: File)->dict:
    db = Session()
    result=userController(db).upload_file_user(usuario)
    # evaluamos el resultado
    estado=result['result']
