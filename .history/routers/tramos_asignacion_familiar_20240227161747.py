...
from controller.tramos_asignacion_familiar import TramosImpuestoUnicoController
...
def create_tramo_asignacion_familiar(tramosAsignacionFamiliar:TramosAsignacionFamiliarSchema, userCreatorId : int = Query (ge=1, le=os.getenv("MAX_ID_USERS")))->dict:
    db = Session()
    result=TramosImpuestoUnicoController(db).create_tramo_asignacion_familiar(tramosAsignacionFamiliar,userCreatorId)
...
def list_tramo_asignacion_familiar()->dict:
    ...
    result = TramosImpuestoUnicoController(db).list_tramo_asignacion_familiar()
...
def get_tramo_asignacion_familiar(id: int):
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = TramosImpuestoUnicoController(db).get_tramo_asignacion_familiar(id)
    # debemnos convertir los objetos tipo BD a Json
    if (result):
        if (result["result"]=="1"):
            data=result['data']
...
def list_history_tramo_asignacion_familiar( id : int =Path(ge=1, lt=1000))->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = TramosImpuestoUnicoController(db).list_history_tramo_asignacion_familiar(id)
...
def search_tramo_asignacion_familiar(finding : str = Query (min_length=os.getenv("MIN_STR_SEARCH_USER"), max_length=os.getenv("MAX_STR_SEARCH_USER")))->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = TramosImpuestoUnicoController(db).search_tramo_asignacion_familiar(finding)
...
def update_tramo_asignacion_familiar(tramosAsignacionFamiliar:TramosAsignacionFamiliarSchema, user_updater: int = Query(ge=1, le=os.getenv('MAX_ID_USERS')), id : int = Path(ge=1,le=1000))->dict:
    ...
    result = TramosImpuestoUnicoController(db).update_tramo_asignacion_familiar(tramosImpuestoUnico, user_updater, id) 
    if (result['result']=="1"):
        data=result['data']
