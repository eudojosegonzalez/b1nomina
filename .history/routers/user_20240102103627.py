...
async def file_upload_user(creatorUserId : int = Query (ge=1, lt=20000) ,id : int = Path (ge=1, lt=20000),file : UploadFile=File())->dict:
...
async def search_users(finding : str = Query (min_length=os.getenv("MIN_STR_SEARCH_USERS"), max_length=os.getenv("MAX_STR_SEARCH_USERS")), page : int = 1, records : int = 20)->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
...
# Funcion para listar los archivos de un usuario del sistema, se giltra por ID
...
def get_file_user(request : Request ,id:int = Path(ge=1, le=os.getenv("MAX_FILES_USERS")))->dict:
    db = Session()
    # almacenamos el listado de usarios en un resultset
    result = userController(db).get_file_user(request,id)
    # debemnos convertir los objetos tipo BD a Json
    if (result):
        if (result["result"]=="1"):
            data=result['resultado']
